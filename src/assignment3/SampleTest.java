...
public class SampleTest {
    ...
    public void bfsTest1() {
	    ArrayList<String> res = Main.getWordLadderBFS("dough", "roast");
        boolean ok = true;
        assertTrue(verifyLadder(res));
    }
    ...
    public void bfsTest2() {
        ArrayList<String> res = Main.getWordLadderBFS("rakus", "twist");
        outContent.reset();
        Main.printLadder(res);
        String str = outContent.toString().replace("\n", "").replace(".", "").trim();

        assertEquals("no word ladder can be found between rakus and twist", str);
    }
    ...
    public void dfsTest1() {
        ArrayList<String> res = Main.getWordLadderDFS("quest", "uncos");
        outContent.reset();
        Main.printLadder(res);
        String str = outContent.toString().replace("\n", "").replace(".", "").trim();
        assertEquals("no word ladder can be found between quest and uncos", str);
    }

    // Has no Ladder
    @Test(timeout = 30000)
    public void dfsTest2() {
        ArrayList<String> res = Main.getWordLadderDFS("taper", "piper");
        if (res != null) {
            HashSet<String> set = new HashSet<String>(res);
            assertEquals(set.size(), res.size());
        }
